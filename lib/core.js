...
import { sha256, clone, toHex, s2b } from './util.js'
...
export default class FatKernel {
  ...
  replicate (isInitator, opts = {}) {
    this.base.replicate(isInitator, opts)
  }

  async beginSwarm (Hyperswarm, topic = 'poh:v1/global', verbose = true) {
    const discoveryKey = await sha256(s2b(topic)) // this.discoveryKey

...
const WORLD_KEY = Buffer.from('world')
...
export default class Core {
  ...
  constructor (store, opts = {}) {
    ...
    this.router.add('@honor/world/genesis', applyWorldGenesis)
  ...
  async createWorld (opts = {}) {
    ...
    await this.base.append(encode('@honor/world/genesis', {
      version: opts.version || 1,
      ruleset: opts.ruleset || 'poh:v1',
      admission: opts.admission || 'open',
      topic: this._worldTopic
    }), this._appendOpts())
  ...
  async readWorld () {
    return getWorld(this.base.view)
  }
...
async function applyWorldGenesis (data, context) {
  const { view, author, seq } = context

  const value = await getWorld(view)
  if (value) return rejectBlock('world already exists')
  if (seq !== 1) return rejectBlock('non-genesis world')
...
async function applySpawnPlayer (data, context) {
  ...
  const world = await getWorld(view)
  const creatorSpawn = world && cmp(world.creator, author) && seq === 2
  if (seq !== 1 && !creatorSpawn) return rejectBlock('non-genesis spawn, seq:', seq)
...
async function getWorld (view) {
  const node = await view.get(WORLD_KEY)
  return node ? unpack(node.value) : null
}

async function putWorld (view, world) {
  const w = view.write()
  w.tryPut(WORLD_KEY, pack(world))
  await w.flush()
}

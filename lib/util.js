...
export function toU8 (o) {
  ...
  throw new Error('Uint8Array coercion failed' + o)
}
...
export function hexdump (bytes, log = console.log, ω = 16) {
  let out = ''
  bytes = toU8(bytes)
  ...
  for (let i = 0; i < bytes.length; i++) {
    const c = bytes[i]
    hstr.push(hexes[c])
    astr.push((c >= 0x20 && c <= 0x7E) ? String.fromCharCode(c) : '.')
    if (i && !(i % ω)) {
      const line = hstr.join(' ') + ' ' + astr.join('')
      hstr = []
      astr = []
...
export function typeOf (o) {
  ...
  if (Buffer.isBuffer(o) || o instanceof Uint8Array) return 'u8'
  return typeof o
}
